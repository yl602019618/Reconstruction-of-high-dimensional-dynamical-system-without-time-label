"""
2D joint inversion with spatially varying diffusion.

True: μ=(x-x³, y-y³), D(x,y) = 0.5 + 0.2*exp(-(x²+y²))
Strategy: two-phase training, N=200, poly+NN for both drift and diffusion.
"""
...
from joint_2d_v2 import (
    solve_fpe_2d_varD, poly_basis_2d, pointwise_w1,
    DriftNet2D, DiffusionNet2D_small, make_setup
)

torch.set_default_dtype(torch.float64)


class DiffusionNet2D_flex(nn.Module):
    """D(x,y) = softplus(b₀ + b₁x + b₂y + b₃x² + b₄y² + b₅xy + NN(x,y))
    Quadratic poly to capture the Gaussian bump shape."""
    def __init__(self, hidden=16):
        super().__init__()
        self.coeffs = nn.Parameter(torch.zeros(6))  # [1, x, y, x², y², xy]
    ...
    def forward(self, X, Y):
        c = self.coeffs
        raw = c[0] + c[1]*X + c[2]*Y + c[3]*X**2 + c[4]*Y**2 + c[5]*X*Y
...
def run(N_obs=200, N_sim=200, M=30, n_substeps=20, seed=0,
    ...
    mu1_true = X - X**3
    mu2_true = Y - Y**3
    D_true = 0.5 + 0.2 * torch.exp(-(X**2 + Y**2))
    ...
    diff_net = DiffusionNet2D_flex(hidden=16).double()
    ...
    # ===== Phase 1: drift only =====
    print("=" * 60)
    print(f"Phase 1: drift only ({phase1_iter} iters)")
    print(f"True D(x,y) = 0.5 + 0.2*exp(-(x²+y²)), N={N_obs}")
    print("=" * 60)
    ...
    for it in range(phase1_iter):
        ...
        rho_sim = solve_fpe_2d_varD(mu1, mu2, D_init, x, y, t_sim, rho0, n_substeps=n_substeps)
    ...
    # Row 3: D
    vmin_D = min(Dt.min(), Dl.min()) - 0.02
    vmax_D = max(Dt.max(), Dl.max()) + 0.02
    kw_D = dict(levels=30, cmap='viridis', vmin=vmin_D, vmax=vmax_D)
    ax = fig.add_subplot(5, 3, 7)
    im = ax.contourf(Xn, Yn, Dt, **kw_D); plt.colorbar(im, ax=ax)
    ax.set_title('True D(x,y)'); ax.set_aspect('equal')

    ax = fig.add_subplot(5, 3, 8)
    im = ax.contourf(Xn, Yn, Dl, **kw_D); plt.colorbar(im, ax=ax)
    ax.set_title('Learned D(x,y)'); ax.set_aspect('equal')

    ax = fig.add_subplot(5, 3, 9)
    im = ax.contourf(Xn, Yn, Dl-Dt, levels=30, cmap='RdBu_r'); plt.colorbar(im, ax=ax)
    ax.set_title(f'Error D (max={abs(Dl-Dt).max():.3f})'); ax.set_aspect('equal')
    ...
    plt.tight_layout()
    plt.savefig('joint_2d_varD.png', dpi=150)
    plt.close()

    print(f"\n{'='*60}")
    print(f"FINAL: μ_mse={mu_mse_hist[-1]:.4e}, D_mse={D_mse_hist[-1]:.4e}")
    print("Saved joint_2d_varD.png")


if __name__ == '__main__':
    run(N_obs=200, N_sim=200, phase1_iter=400, phase2_iter=400)
